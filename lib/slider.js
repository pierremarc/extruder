...
import { identity, clamp } from 'lodash/fp';
import { createElement, appendText, px, addClass, removeClass } from './dom';
...
function startHandler(key, parser, line, square) {
    return (() => {
        ...
        addClass(line, 'is-moving');
        addClass(square, 'is-moving');
...
function stopHandler(key, parser, line, square) {
    return ((e) => {
        ...
        if (state.started) {
            ...
            state.started = false;
            removeClass(line, 'is-moving');
            removeClass(square, 'is-moving');
...
export default function slider(options) {
    ...
    const lineWrapper = createElement('div', { class: 'slider-wrapper' });
    ...
    const sureParser = parser || identity;
    const clampedParser = (v) => clamp(min, max, sureParser(v));

    const start = startHandler(key, clampedParser, lineWrapper, square);
    const stop = stopHandler(key, clampedParser, lineWrapper, square);
    const move = moveHandler(key, clampedParser, lineWrapper, square);
    const cancel = cancelHandler(key, clampedParser, lineWrapper, square);
    ...
    input.addEventListener('change', () => {
        setState(key, clamp(min, max, Number(input.value)));
    });
