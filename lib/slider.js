...
export default function slider(options) {
    ...
    const clampedParser = (v) => clamp(min, max, parser(v));

    const start = startHandler(key, clampedParser || identity, lineWrapper);
    const stop = stopHandler(key, clampedParser || identity, lineWrapper, square);
    const move = moveHandler(key, clampedParser || identity, lineWrapper, square);
    const cancel = cancelHandler(key, clampedParser || identity);
